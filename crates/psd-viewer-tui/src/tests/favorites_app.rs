...
use crate::{
    is_favorite_save_key, is_favorites_toggle_key, is_help_overlay_close_key,
    is_log_overlay_close_key,
};
...
#[test]
fn help_overlay_close_key_accepts_plain_esc_only_when_visible() {
    let plain_esc = KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE);
    let ctrl_esc = KeyEvent::new(KeyCode::Esc, KeyModifiers::CONTROL);

    assert!(!is_help_overlay_close_key(&plain_esc, false));
    assert!(is_help_overlay_close_key(&plain_esc, true));
    assert!(!is_help_overlay_close_key(&ctrl_esc, true));
}
