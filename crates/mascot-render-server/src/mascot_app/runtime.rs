            ...
            .show(ctx, |ui| {
                ...
                if self.transparent_input_flash_enabled()
                    ...
                    && response.interact_pointer_pos().is_some_and(|pos| {
                        // Blue flash is visual feedback for transparent input, so it stays
                        // enabled even when precise pointer interaction is disabled during
                        // the always-bend animation.
