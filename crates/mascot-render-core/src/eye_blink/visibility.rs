...
pub(super) fn ensure_named_row_visible(
    ...
    let rows = resolve_row_visibility(document, &variation);
    ...
    let rows = resolve_row_visibility(document, &variation);
    if rows.get(row_index).is_some_and(|visible| *visible) {
        return Ok(variation);
    }
...
pub(super) fn find_active_eye_blink_pair(
    document: &PsdDocument,
    base_variation: &DisplayDiff,
    ...
    for (closed_row_index, closed_descriptor) in document.layers.iter().enumerate() {
        if !is_named_exclusive_descriptor(closed_descriptor, &target.second_layer_name)
            || !states
        ...
        let Some((open_row_index, score)) =
            active_open_row_in_scope(document, &states, target, closed_row_index)
        else {
            continue;
        };
        ...
        match best_match {
            Some((_, _, best_score, best_distance))
                if best_score > score || (best_score == score && best_distance <= distance) => {}
            _ => best_match = Some((open_row_index, closed_row_index, score, distance)),
        }
    }

    best_match.map(|(open_row_index, closed_row_index, _, _)| (open_row_index, closed_row_index))
}

pub(super) fn find_named_exclusive_pair(
    document: &PsdDocument,
    first_name: &str,
    second_name: &str,
) -> Option<(usize, usize)> {
...
fn resolve_row_visibility(document: &PsdDocument, display_diff: &DisplayDiff) -> Vec<bool> {
    resolve_row_states(document, display_diff)
        .into_iter()
...
fn active_open_row_in_scope(
    ...
    let mut preferred_visible = None;
    ...
    for row_index in scope_start..scope_end {
        ...
        if descriptor.depth != closed_descriptor.depth || !is_exclusive_descriptor(descriptor) {
            continue;
        }

        let matches_default = normalized_layer_name(&descriptor.name) == target.first_layer_name;
        if matches_default {
            preferred_any = Some(row_index);
        }
        if states.get(row_index).is_some_and(|state| state.visible) {
            any_visible.get_or_insert(row_index);
            if matches_default {
                preferred_visible = Some(row_index);
            }
        }
    }

    preferred_visible
        .map(|row_index| (row_index, 3))
        .or_else(|| any_visible.map(|row_index| (row_index, 2)))
        .or_else(|| preferred_any.map(|row_index| (row_index, 1)))
}
